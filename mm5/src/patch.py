...
OVERLAY_EXTRA = 36
PERSONAL_MESSAGE = "なんでもできる！".encode("utf-16le")
DECOI_KEY = [int.from_bytes(PERSONAL_MESSAGE[offset:offset+0x2], 'little') for offset in range(0x0, len(PERSONAL_MESSAGE), 0x2)]
KEY_LEN = len(DECOI_KEY)
...
def decoi_encrypt(data: bytes):
  # Encryption used in MM5
  # Nothing fancy, just a smol little XOR in halfwords
  encrypted_data = bytearray(data)
  size = len(encrypted_data)
  for i in range(size >> 1):
    offset = i << 1
    encrypted_data[offset:offset+0x2] = (int.from_bytes(encrypted_data[offset:offset+0x2], 'little') ^ DECOI_KEY[i % KEY_LEN]).to_bytes(2, 'little')
    ...
    if readline==0:
      ...
      if len(section)>0:
        if section[1].startswith(".text"): # Retrieve only text sections
          ...
          # If Overlay1, it may need to to be encrypted...
          if bank_number == 1 and section[1].endswith("TOPMENU"):
            custom_code_bytes = decoi_encrypt(custom_code_bytes)
...
# oh yeah right we also have to encrypt part of arm9
# too lazy to do something modular
ARM9_SPOOKY_START = 0x5E5D4
ARM9_SPOOKY_END = rom.arm9[ARM9_SPOOKY_START:].find((0xDEADBEEF).to_bytes(4, 'little'))
if ARM9_SPOOKY_END < 0:
  raise RuntimeError("help i cant find the super secret arm9 area ;-;")
ARM9_SPOOKY_END += ARM9_SPOOKY_START + 0x4
arm9 = bytearray(rom.arm9)
arm9[ARM9_SPOOKY_START:ARM9_SPOOKY_END] = decoi_encrypt(arm9[ARM9_SPOOKY_START:ARM9_SPOOKY_END])
rom.arm9 = bytes(arm9)
